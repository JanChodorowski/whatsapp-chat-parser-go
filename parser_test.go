...
func TestParseString(t *testing.T) {
	...
	for _, chatExample := range chatExamples {
		...
		t.Run("Get people from messages "+chatExample.description, func(t *testing.T) {
			messages, err := ParseString(string(fileContents), nil)
			if err != nil {
				t.Errorf("Unexpected error: %v", err)
			}

			people := GetPeopleFromMessages(&messages)

			test := areStringArraysIdenticalIgnoringOrder(people, chatExample.people)

			if !test {
				t.Errorf("Expected people to be %v, got %v", chatExample.people, people)
			}
...
var chatExamples = []chatTestExample{
	{
		...
		people:        []string{"Sample User", "TestBot", "+410123456789"},
	...
	},
	{
		...
		people:        []string{"Andrew", "Josh", "Marthy"},
	...
	},
	{
		...
		people:        []string{"+33 6 99 88 77 66", "+48 777 666 555", "Andrew"},
